...
public class ElevatorSubsystem extends SubsystemBase {
  ...
  public Command moveToPositionCommand(double position) {
    return Commands.sequence(
      ...
      Commands.runOnce(() -> Logger.recordOutput("Elevator/Status", "Reached position: " + position))
    );
  }
  ...
  public void periodic() {
    ...
    if (Math.abs(voltage) > 0.1 && Math.abs(currentPosition - lastPosition) < Elevator.STALL_POSITION_THRESHOLD) {
      stallCount++;
