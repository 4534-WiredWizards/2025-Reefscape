...
public class RobotContainer {
        ...
        private PathPlannerPath getPathForZoneAndSide(ReefZone zone, ScoringSide side) {
                ...
                PathPlannerPath path = switch (zone) {
                        case ZONE_1 ->
                                side == ScoringSide.RIGHT ? Z1R : Z1L;
                        case ZONE_2 ->
                                side == ScoringSide.RIGHT ? Z2R : Z2L;
                        case ZONE_3 ->
                                side == ScoringSide.RIGHT ? Z3R : Z3L;
                        case ZONE_4 ->
                                side == ScoringSide.RIGHT ? Z4R : Z4L;
                        case ZONE_5 ->
                                side == ScoringSide.RIGHT ? Z5R : Z5L;
                        case ZONE_6 ->
                                side == ScoringSide.RIGHT ? Z6R : Z6L;
                        default ->
                                null;
                };
        ...
        /**
         * Creates an auto scoring sequence using the current zone and specified
         * side.
         *
         * @param side The scoring side (LEFT or RIGHT)
        ...
        public Command autoScoringSequence(ScoringSide side, ScoringHeight height) {
                return new SequentialCommandGroup(
                                new InstantCommand(() -> {
                                        System.out.println("[AutoScore] Starting sequence");
                                        System.out.println("-> Requested Side: " + side + " | Height: " + height);
                                        System.out.println("-> Initial Zone: " + drive.getZone());
                                        System.out.printf("-> Initial Pose: X: %.2f Y: %.2f Rot: %.1f°\n",
                                                        drive.getPose().getX(),
                                                        drive.getPose().getY(),
                                                        drive.getPose().getRotation().getDegrees());
                                }),
                                new InstantCommand(() -> {
                                        System.out.println("\n[AutoScore] Resetting pose via vision...");
                                        vision.resetRobotPose();
                                }),
                                new InstantCommand(() -> {
                                        System.out.println("\n[AutoScore] After pose reset:");
                                        System.out.println("-> Current Zone: " + drive.getZone());
                                        System.out.printf("-> Current Pose: X: %.2f Y: %.2f Rot: %.1f°\n",
                                                        drive.getPose().getX(),
                                                        drive.getPose().getY(),
                                                        drive.getPose().getRotation().getDegrees());
                                }),
                                new InstantCommand(() -> {
                                        System.out.println("\n[AutoScore] Path selection:");
                                        System.out.println("-> Alliance: "
                                                        + DriverStation.getAlliance().orElse(Alliance.Blue));
                                        System.out.println("-> Current Zone: " + drive.getZone());
                                        System.out.println("-> Requested Side: " + side);

                                        PathPlannerPath path = getPathForZoneAndSide(drive.getZone(), side);
                                        if (path != null) {
                                                System.out.println("-> Selected Path: " + path.name);
                                                System.out.printf("-> Path Start: X: %.2f Y: %.2f\n",
                                                                path.getPoint(0).position.getX(),
                                                                path.getPoint(0).position.getY());
                                        } else {
                                                System.err.println("!! ERROR: No path found for zone " + drive.getZone()
                                                                + " side " + side);
                                        }
                                }),
                                new ParallelCommandGroup(
                                                new SetWristPosition(m_Wrist, Wrist.MIN_CLEAR_ELEVATOR_ANGLE, true)
                                                                .andThen(() -> System.out.println(
                                                                                "\n[AutoScore] Wrist cleared elevator")),
                                                new DriveToPath(drive, getPathForZoneAndSide(drive.getZone(), side))
                                                                .andThen(() -> System.out.println(
                                                                                "\n[AutoScore] Finished driving to path"))),
                                new RunCoralOutake(m_Intake)
                                                .andThen(() -> System.out
                                                                .println("\n[AutoScore] Coral outtake complete")),
                                new SetWristPosition(m_Wrist, Wrist.MIN_CLEAR_ELEVATOR_ANGLE, true)
                                                .andThen(() -> System.out.println("\n[AutoScore] Sequence complete!")));
        }
                                                                ...
                                                                new ParallelDeadlineGroup(
                                                                                ...
                                                                                new SequentialCommandGroup(
                                                                                                new WaitUntilCommand(
                                                                                                                () -> m_elevator
                                                                                                                                .getEncoderPosition() < Elevator.ELEVATOR_DANGER_LIMIT),
    ...
    private void configureButtonBindings() {
        ...
        public void configureCoralAutoScoringButtons() {
                ...
                Trigger leftLevel2 = operatorController
                                .axisLessThan(Operator.RIGHT_THUMB_AXIS, -0.3)
                                .and(operatorController.povLeft());
                Trigger leftLevel3 = operatorController
                                .axisLessThan(Operator.RIGHT_THUMB_AXIS, -0.3)
                                .and(operatorController.povRight());
                Trigger leftLevel4 = operatorController
                                .axisLessThan(Operator.RIGHT_THUMB_AXIS, -0.3)
                                .and(operatorController.povUp());

                // Right Side
                Trigger rightLevel1 = operatorController
                                .axisGreaterThan(Operator.RIGHT_THUMB_AXIS, 0.3)
                                .and(operatorController.povDown());
                Trigger rightLevel2 = operatorController
                                .axisGreaterThan(Operator.RIGHT_THUMB_AXIS, 0.3)
                                .and(operatorController.povLeft());
                Trigger rightLevel3 = operatorController
                                .axisGreaterThan(Operator.RIGHT_THUMB_AXIS, 0.3)
                                .and(operatorController.povRight());
                Trigger rightLevel4 = operatorController
                                .axisGreaterThan(Operator.RIGHT_THUMB_AXIS, 0.3)
                                .and(operatorController.povUp());
